...
--[[
ALL OF THE CODE BELOW IS BEING TESTED AND USED FOR NEW makeCHAR SCRIPT

!!!DO NOT DELETE
]]

-- presets newChar, name1, name2

function pickTemplate(choice)
    if tostring(choice) == "newChar" then
        -- make new char
    else
        -- make whatever template is chosen
...
function findMorphs()
    for  x, y in pairs(character:GetChildren()) do
        ...
        local newTop = string.format(topStr, y)
        if newTop == "Top" then
            deleteTop(newTop)
        end
    end
...
function deleteTop(topToDelete)
    local top = "game." ..tostring(topToDelete:GetFullName())
    print(top)

    -- make metatable(?) and add top to it
    local topTable = { }
end

-- do the same, but for Bottom too
